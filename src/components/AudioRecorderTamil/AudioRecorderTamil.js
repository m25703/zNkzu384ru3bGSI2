...
function Mic({
  ...
  value,
  flag,
  setHindiRecordedAudio,
  setHindiRecordedText,
}) {
  ...
  const getASROutput = (asrInput, blob) => {
    ...
    fetch(ASR_REST_URL, requestOptions)
      ...
      .then(result => {
        ...
        setHinditext(apiResponse['output'][0]['source']);
        createDownloadLink(blob, apiResponse['output'][0]['source']);
        stopLoading();
        setHindiRecordedText(apiResponse['output'][0]['source']);
      })
      .catch(error => {
